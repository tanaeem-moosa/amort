...
- **Linear Graph Traversals & Handshaking (BFS)**: `Amort.Graph.Traversal` formalizing adjacency list representations, the directed Handshaking Lemma $\sum_{v} \text{outdeg}(v) = |E|$, executable queue/visited BFS loop `bfsLoop`, canonical two-sided shortest path distance specification (`bfsDist_eq_top_iff`, `bfsDist_eq_coe_iff`), algorithm distance equivalence (`bfsWithCount_fst_eq`), distance walk soundness `bfsWithCount_walk`, fuel invariance `bfsLoop_fuel_invariant`, and in-loop instrumented execution counter `bfsWithCount` bounded by $|V| + |E|$ unconditionally without clamping. Documented in [`Amort/Graph/Traversal.md`](Amort/Graph/Traversal.md).
