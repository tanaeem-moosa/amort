...
/-- Aho-Corasick text scanning operational work $2|T|$ is asymptotically $O(|T|)$
under `Filter.atTop`. -/
...
/-- Aho-Corasick total multi-pattern search operational work $\sum |P_i| + 2|T| + z$
...
theorem isBigO_acTotalSearchWork_atTop :
  ...
  rintro ⟨⟨P, T⟩, z⟩
  simp only [Real.norm_eq_abs, Nat.abs_cast]
  dsimp [AhoCorasick.acTotalSearchWork]
  push_cast
  linarith
